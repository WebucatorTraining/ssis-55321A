 ...
 **\Studentfiles** Make note of the full path to your new project. Set your project
 ...
 SQL Server 2016.
    ...
    3. In the New Project window, expand `Business Intelligence > Integration Services`. Select
     `Integration Services Project`. Change the Name to **Ch4LabEx1**. Verify that
     `Create directory for solution` is checked, and then click `OK`.
    4. In the Solution Explorer, click `Ch4LabEx1`. In the Properties window, hover over
     **FullPath** and write down the path to your solution folder. You will be moving a file to this
     location later.
    5. Right-click the Ch4LabEx1 project, and then click `Properties`.
    6. On the Configuration Properties page, change the TargetServerVersion to **SQL Server 2016**,
     and
     then click `OK`.
    7. Click `Yes` to accept the change, and then click `Yes to All` if prompted to reload
     the
     files.
2. Rename the default **Package.dtsx** file to **CopyFile.dtsx**.
    1. In the Solution Explorer, expand `SSIS Packages` (if necessary), Right-click **Package.dtsx** and
     select `Rename`. Rename the package **CopyFile.dtsx**.
...
4. Set the following properties for the Copy File task.
    ...
    2. Set the **SourceConnection** to a new connection manager that points to the existing file **Chapter 04 Common Tasks\Labs\Starters\ ProductsBlue.csv**.
        ...
        2. In the File Connection Manager Editor, select `Existing File` from the Usage type dropdown
         list. Click the `Browse` button and navigate to **\Chapter 04 Common
         Tasks\Labs\Starters\ProductsBlue.csv,** and then click `Open`.
        3. Click `OK` to close the File Connection Manager Editor.
    3. Set the `DestinationConnection` to a new connection manager that points to the existing folder
     that you wrote down in step 1 above.
        1. In the Destination Connection section of the General page in the File System Task Editor, set
         `DestinationConnection` to `<New Connection…>.`
        2. In the File Connection Manager Editor, select `Existing folder` from the Usage type
    ...
    4. Configure the destination so that a file can be copied multiple times and simply overwrite the existing file
     each time.
        ...
        2. Click `OK` to close the File System Task Editor.
5. Execute just this new task to test it. Once the task has completed successfully, leave debug mode. If the
 task failed, double check your settings and execute the task again.
    1. Right-click the `Copy File` task and select `Execute Task` to test that the
...
If you are using a corporate email server, the To and From addresses should both be your
 email address.
    ...
    5. Click `OK` to close the Send Mail Task Editor.
7. Execute the entire package. The Send Mail Task should not execute. Set the Copy File task to report a failure
 ...
 task ran successfully.
    1. Click the `Start` button to run the package. A green check mark should appear at the corner of
     the first
     task.
    2. Click the `Stop Debugging` button (![Stop debugging button](Images/ssis-stop-debugging-button.png)) to return to Design mode.
    ...
    5. Click the `Start` button to run the package. Although the Copy File task completes successfully,
     you will
    ...
    6. Click the `Stop Debugging` button (![Stop debugging button](Images/ssis-stop-debugging-button.png)) to return to Design mode.
...
1. Use SQL Server Management Studio to run the **Ch4\_CreateView.sql** script in the **Chapter 04 Common Tasks\Labs\Starters\** folder. This will create a view that includes
 both reseller sales data and country codes.
    ...
    2. Click `Connect`.
    3. Click `Open File` on the toolbar (alternatively, select **File > Open > File**  from the menu.)
    4. Browse to the **Chapter 04 Common Tasks\Labs\Starters\Ch4\_CreateView.sql**, and
     then click `Open`.
    5. Click `Execute` on the toolbar. The status bar should report Query executed successfully.
...
3. Add an ADO.NET project connection manager to the AdventureWorksDW database.
    ...
    2. In the Add SSIS Connection Manager dialog box, select `ADO.NET`, and then click
     `Add`.
    3. In the Configure ADO.NET Connection Manager dialog box, click `New`.
    4. In the Connection Manager dialog box:
        ...
        3. In the Select or enter database name dropdown list, select **AdventureWorksDW**, and then
         click `OK`.
    5. Click `OK` to close the Configure ADO.NET Connection Manager dialog box.
4. Create a new package named **Data Profiling** in the **Ch4LabEx1** project that you
 created in Exercise 1. If you did not complete exercise 1, you can use the solution located at **Chapter 04 Common Tasks\Labs\Answers\Ch4LabEx1**.
    1. In the Solution Explorer, right-click `SSIS Packages` and select **New SSIS
     Package**.
...
5. Add a **Data Profiling Task** to the control flow that uses a new file connection as the
 ...
 requests as described in the following steps.
    ...
    2. Drag a `Data Profiling Task` from the Common area of the SSIS Toolbox to the design
     surface.
    3. Right-click the task and choose `Edit`.
    ...
    7. In the File Connection Manager Editor, set Usage type to **Create file**. Click
     `Browse` and navigate to the folder that contains your solution. Enter a File name of
     **DataProfile.xml**, and then click `Open`. Click `OK` to
...
7. Configure a Column Value Distribution Profile Request with the following settings:
    ...
    4. ValueDistributionOption – AllValues
        1. Select `Column Value Distribution Profile Request` from the Profile Type drop-down
         list.
...
9. Configure a Column Statistics Profile Request with the following settings:
    ...
    3. Column – SalesAmount
        1. Select `Column Statistics Profile Request` from the Profile Type drop-down
         list.
        2. Configure the Request Properties as specified above.
        3. Click `OK` to close the Data Profiling Task Editor.
10. Execute the DataProfiling project. Use the Profile Viewer in the Data Profiling Task Editor to review
 the information collected.
    1. Click `Start` (![Start button](Images/ssis-start-button.png)) to
     run the task.
     Upon completion, the task should have a green check mark (![green check mark](Images/ssis-green-checkmark.png)).
    2. Click the `Stop Debugging` button (![Stop debugging button](Images/ssis-stop-debugging-button.png)) to return to Design mode.
    3. Right-click the Data Profiling task and choose `Edit`.
    4. In the Data Profiling Task Editor, click `Open Profile Viewer`. Explore the information
...
1. Use SQL Server Data Tools to open the **Chapter 04 Common
 Tasks\Labs\Starter\MasterPackage\MasterPackage.sln** solution file.
    ...
    2. Browse to **Chapter 04 Common
     Tasks\Labs\Starter\MasterPackage\MasterPackage.sln**, and then click `Open`.
2. Add six Execute Package Tasks to the Control Flow of the **MasterPackage.dtsx**
 package.
    1. In Solution Explorer, right-click the **MasterPackage.dtsx** package, and then
     click `Open`.
    2. Drag six `Execute Package Task` items from the Common section of the SSIS Toolbox to the
...
3. Name, arrange, and edits the tasks and precedence constraints to meet the following goals.
    1. Tasks should be named to reflect the package that they are executing.
        1. Right-click each `Execute Package` task, and then click `Rename`.
        ...
        3. For each task, right-click the task, and then click `Edit`. In the Execute Package Task
         Editor
         dialog box, change to the **Package** page, select the appropriate package name from the
         PackageNameFromProjectReference drop-down list, and then click `OK`
...
4. Execute **MasterPackage.dtsx** in Debug mode. Notice that each of the packages opens
 ...
 Review the pop-up dialog boxes from the individual packages. Click `OK` on one message at a time, and
 ...
 **MasterPackage.dtsx** package again and review the tasks as the package runs. click
 ...
 Debug mode.
    ...
    4. Return to the **MasterPackage.dtsx** designer and review the results of the
     execution.
    5. Leave Debug mode.
7. Click `Save all` (![save all](Images/ssis-save-all.png)), and then
...
The message boxes are used in this example to make the packages pause so that you can see the flow of when the
 ...
 no
 one will be there to click `OK` on your automated packages.