...
1. Create two new folders in the **Studentfiles** folder, one named **Chapter
 10Dev,** and another named **Chapter 10QA**.
    ...
    2. On the right side on Windows Explorer, right-click a blank area, and then click `New` >
...
2. If one does not already exist on your SQL Server instance, create the SSIS Catalog.
    ...
    2. In the Connect to Server dialog box, verify that the Server type is set to **Database Engine**,
     your
     server name is entered, and the proper authentication is set, and then click `Connect`.
    3. In Object Explorer, right-click the **Integration Services Catalogs** folder, and then click
     `Create Catalog`.
    4. In the Create Catalog dialog box, click `Enable CLR Integration`.
    5. Click `Enable automatic execution of Integration Services stored procedure at SQL Serve startup`.
...
3. In SQL Server Management Studio, create a new folder named **Chapter 10Lab** in the SSIS
 Catalog.
    ...
    3. In Object Explorer, expand **Integration Services Catalogs**, right-click the **SSISDB
     catalog**, and then click `Create Folder`.
...
5. Deploy the project to the Chapter 10Lab folder on your SSIS Catalog.
    ...
    You may get a warning message: Failed to decrypt an encrypted XML node. By default, SSDT encrypts
     ...
     deployment.
    3. On the Select Destination page, type in your server name, or click `Browse`, click your server,
     and
     then click `OK`. Click Connect. In the path box, click `Browse`, click the
     **Chapter
     10Lab** folder, and then click `OK`. Notice that the path includes the project name after
     the folder
     that you selected.
    4. Click `Next`.
    5. Review your selections, and then click `Deploy`.
    6. Once the deployment has succeeded, click `Close`, and then return to SQL Server Management
     Studio.
 ...
 **Dev** and **QA**.
    ...
    2. In the **Environment name** box, type **Dev**, and then click `OK`.
 ...
 **DevAdmin@adventureworks.com**.
    1. Under the Environments folder in Object Explorer, right-click `Dev`, and then click
     `Properties`.
    2. In the Environment Properties dialog box, click `Variables` in the Select a page section.
    3. Create three new variables as shown in Figure 16, and then click `OK`.
 ...
 **QAAdmin@adventureworks.com**.
    1. Under the Environments folder in Object Explorer, right-click `QA`, and then click
     `Properties`.
    2. In the Environment Properties dialog box, click `Variables` in the Select a page section.
    3. Create three new variables as shown in Figure 17, and then click `OK`.
...
11. Configure the Lab7 project to reference the Dev and QA environments.
    1. In Object Explorer, expand **Integration Services Catalogs > SSISDB > Chapter 10Lab >
     Projects** and right-click the **Lab7** project, and then click `Configure`.
    2. In the Configure – Lab7 dialog box, click `References` in the Select a page area.
    3. On the References page, click `Add`, click `Dev`, and then click `OK`.
...
12. Map the variables that you created in Step 8 to the project parameters in the **Lab5** project.
    1. In the Configure – Lab7 dialog box, click `Parameters` in the Select a page area, and then select
    ...
    3. In the Set Parameter Value dialog box, click the radio button for **Use environment variable**,
     select **Department** from the drop-down list, and then click `OK`.
    4. Repeat this process to map the DestinationPath and ProjEmailToAddress varibles.
    5. Click `OK` to close the Configure – Lab7 dialog box.
13. Execute the **DynamicPackage.dtsx** package, associate the execution with one of the
 ...
 the proper department.
    ...
    4. Click `Yes` in the SSMS message that prompts you to open the Overview Report now. Review the
     report to
     verify the environment settings were used during the package execution. You can review the parameter settings
     in the
    ...
    5. Open Windows Explorer and Browse to the **C:\Chapter 10Dev** folder to verify that
     a
     **SalesDepartmentHistory.csv** file exists.
    6. Go to your mail program or the mail drop folder to verify that there is an email message addressed to
     DevAdmin@adventureworks.com. If you are using the Windows built-in SMTP service, you should find the message
     at
 ...
 **DynamicPackage.dtsx** package from the SSIS catalog on your server. The package should
 ...
 and the environment id.
    ...
    5. Locate the environment ID for the Dev environment by expanding **Integration Services Catalogs >
     SSISDB
     > Chapter 10Lab > Environments**, right-clicking `Dev`, and selecting
     **Properties**.
...
4. Execute your batch file, and then verify the results using the messages in the command prompt window as well as
 the SSIS execution reports in SSMS.
    1. In the Command Prompt window, change to the Classfiles folder and type `CD Studentfiles` and then
     press `Enter`.
    ...
    6. Right-click `Chapter 10Lab`, and then click `Reports > Standard Reports > All
     Executions`.
